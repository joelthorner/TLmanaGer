...
// APENDS
if (!$('#CSS_TLG_CSS').length) $('body').append(_SVG + _CSS);
$('body').append(_BLOCK);
...
function arrowCustomRender(connect, tlgBlockId){
	var $block = $(tlgBlockId),
		 ...
		 blockTop = parseInt($block.css('top')),
		 textTop = parseInt($text.css('top')),

		 blockLeft = parseInt($block.css('left')),
		 textLeft = parseInt($text.css('left')),

		 blockRight = blockLeft + $block.outerWidth(true),
		 textRight = textLeft + $text.outerWidth(true),

		 blockBottom = blockTop + $block.outerHeight(true),
		 textBottom = textTop + $text.outerHeight(true),

		 elMinTop = blockTop < textTop ? blockTop : textTop,
		 elMinLeft = blockLeft < textLeft ? blockLeft : textLeft,

		 elMaxRight = blockRight > textRight ? blockRight : textRight,
		 elMaxBottom = blockBottom > textBottom ? blockBottom : textBottom,

		 elMinRight = elMaxRight - elMinLeft,
